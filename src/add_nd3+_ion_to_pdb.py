"""
Add Nd3+ ions to lanmodulin PDB at EF-hand binding pocket centroids.
...
the centroid of those oxygens, and writes a HETATM record for Nd3+ at
each centroid.  The output PDB can be opened directly in ChimeraX.

Usage:
    python add_nd_to_pdb.py
    python add_nd_to_pdb.py --input my_structure.pdb --ion La
"""
...
def add_metal_ions(input_pdb, output_pdb, ion_symbol="ND", element="ND",
    ...
    for loop_name, (start, end) in ef_hands.items():
        ...
        print(f"    Average Nd-O distance: {avg_dist:.3f} A")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", "-i",
        default="best_Nd_lanmodulin.pdb",
        help="Input PDB file (default: best_Nd_lanmodulin.pdb)",
    )
    parser.add_argument(
        "--output", "-o",
        default=None,
